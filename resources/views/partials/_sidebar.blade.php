<aside x-show="sidebarOpen" @click.away="if (window.innerWidth < 768) sidebarOpen = false"
    ...
    <nav class="space-y-2">
        <a href="{{ route('admin.dashboard.main') }}"
            class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link {{ request()->routeIs('admin.dashboard') ? 'active' : '' }}">
        ...
        {{-- <div>
            ...
            <a href="{{ route('admin.users.index') }}"
                class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link {{ request()->routeIs('admin.users.index') || request()->routeIs('admin.users.activity') ? 'active' : '' }}">
        ...
        @hasrole('moderator', 'super-admin')
            <div>
                ...
                <a href="{{ route('admin.moderation.dashboard') }}"
                    class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link {{ request()->routeIs('admin.moderation.dashboard') ? 'active' : '' }}">
                ...
                <a href="{{ route('admin.moderation.reports') }}"
                    class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link {{ request()->routeIs('admin.moderation.answers') ? 'active' : '' }}">
        ...
        @hasrole('content-manager', 'super-admin')
            <div>
                ...
                <a href="{{ route('admin.subjects.index') }}"
                    class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link {{ request()->routeIs('admin.subjects.index') ? 'active' : '' }}">
        ...
        @hasrole('comunity-manager', 'super-admin')
            <div>
                ...
                <a href="{{ route('admin.announcements.index') }}"
                    class="flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link {{ request()->routeIs('admin.announcements.index') ? 'active' : '' }}">
        ...
        <div class="pt-4 mt-4 border-t border-gray-700">
            <form id="logout-form" action="{{ route('admin.logout') }}" method="POST" class="w-full">
                @csrf
                <button
                    type="submit"class="w-full flex items-center space-x-2 py-2.5 px-4 rounded transition duration-200 sidebar-link hover:bg-red-700">
